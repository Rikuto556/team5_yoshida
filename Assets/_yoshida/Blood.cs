...
public class BloodEffect : UniqueEffect
{
    ...
    public override void Execute(Card card, Card flontCard, Battler player,Enemy enemy,Text message)
    {
        ...
        var battler = FindAnyObjectByType<Battler>();



        if ((player.Life + bloodValue) > battler.LifeMax)
        {
            player.Life = battler.LifeMax;
        }
        else if ((player.Life + bloodValue) <= battler.LifeMax)
        {
            player.Life += damage;
        }
        else
        {
            Debug.LogWarning("battlerが見つかりませんでした");
        }
